...
test("workflow app services smoke wires core panels and late services together", async () => {
  ...
  try {
    const els = createElementsProxy({
      ...
      selectedNodePolicyInfo: { textContent: "" },
    ...
    const store = {
      state: { graph: { name: "Flow Smoke", workflow_id: "wf_1", version: "1.0.0", nodes: [{ id: "n1", type: "clean_md" }], edges: [] } },
      setWorkflowName: (name) => savedNames.push(name),
      exportGraph: () => ({ workflow_id: "wf_1", version: "1.0.0", name: "Flow Smoke", nodes: [{ id: "n1", type: "clean_md" }], edges: [] }),
      addNode: () => "n1",
      getNode: () => null,
      getEdge: () => null,
    };
