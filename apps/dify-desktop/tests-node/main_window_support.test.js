...
test("workflow window allows responsive canvas layout widths", () => {
  ...
  assert.equal(created[0].options?.minHeight, 680);
  assert.deepEqual(created[0].loaded?.loadOptions?.query, { debug: "1", debugAuthorized: "1", legacyAdmin: "1", compatAdminAuthorized: "1" });
});
...
test("workflow admin argv opens explicit legacy admin mode", () => {
  ...
  assert.equal(created.length, 1);
  assert.deepEqual(created[0].loaded?.loadOptions?.query, { legacyAdmin: "1", compatAdminAuthorized: "1" });
});
