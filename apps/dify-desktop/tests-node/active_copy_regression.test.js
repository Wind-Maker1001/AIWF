...
test("workflow template copy stays readable in workflow app", () => {
  const workflowApp = readRepoFile("apps/dify-desktop/renderer/workflow/app.js");

  assert.match(workflowApp, /辩论证据流水线 v1/);
  assert.match(workflowApp, /财报清洗流水线 v2/);
  assert.match(workflowApp, /group: "基础"/);
  assert.match(workflowApp, /group: "AI 参数"/);
  assert.match(workflowApp, /label: "导出熟肉包"/);
  assert.match(workflowApp, /label: "审核键"/);

  assert.doesNotMatch(workflowApp, /杈╄璇佹嵁娴佹按绾/);
  assert.doesNotMatch(workflowApp, /鍩虹/);
  assert.doesNotMatch(workflowApp, /榛樿鎵瑰噯/);
});
...
test("key docs keep cleaned onboarding copy", () => {
  ...
  assert.doesNotMatch(quickstartDesktop, /绂荤嚎鏈湴妯/);
  assert.doesNotMatch(goalFreeze, /宸ヤ綔娴佺紪鎺/);
});
