...
test("local workflow store schema version gate passes", () => {
  const repoRoot = path.resolve(__dirname, "../../..");
  const script = path.resolve(repoRoot, "ops/scripts/check_local_workflow_store_schema_versions.ps1");
  ...
  assert.equal(payload.legacyReads.workflow_node_cache, true);
