...
- reason: default mode rollout still keeps a Python legacy fallback when Rust v2 is unavailable or returns an execution error
- added_at: 2026-03-31
- remove_by: 2026-06-30
- success_metric: shadow compare mismatch rate stays below release threshold; rust error rate stays below release threshold; rollout gate passes in three consecutive release or package audits
- kill_condition: once rollout thresholds stay green, remove Python fallback from the default path or demote it to an explicit exception-only recovery path
