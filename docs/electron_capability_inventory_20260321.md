...
| Audit timeline / failure summary / review history | covered (`compat-hidden`) | `renderer/workflow/audit-*.js`, `review-queue-*.js` | covered | backendize audit/query model; frontend consumes query APIs | governance/backend | 2026-04-19 | WinUI timeline/failure/audit viewer plus governance audit export and review history filters/export reached primary-path parity on 2026-05-29 |
| Review queue and manual review admin surface | covered (`compat-hidden`) | `renderer/workflow/review-queue-*.js` | covered | keep manual decision UI in frontend; backendize review state and transitions | governance/frontend + backend | 2026-04-19 | WinUI manual review queue/history plus review history date filters/export and approve-reject auto-resume parity landed by 2026-05-30 |
| Sandbox alerts / rules / autofix management | covered (`compat-hidden`) | `renderer/workflow/sandbox-*.js`, `panels-ui-governance-*` | covered | backendize policy engine/versioning; keep rule editing, mute, and override UI in frontend | governance/backend + frontend | 2026-05-19 | WinUI sandbox rules/autofix editor plus preset apply/import/export, alert/health viewer, and sandbox audit export parity landed on 2026-06-02 |
...
| Queue control / run history admin panels | covered (`compat-hidden` for admin controls) | `renderer/workflow/run-queue-*.js`, `panels-ui-*.js` | covered | backendize queue control semantics; keep operator control panel in frontend | workflow/platform backend + frontend | 2026-04-19 | WinUI local queue admin surface with pause/resume, task cancel/retry, current flow/canvas queueing, and run history replay landed on 2026-06-03 |
| Quality rule set center | covered (`compat-hidden`) | `renderer/workflow/quality-rule-set-*.js` | covered | backendize rule storage/evaluation/versioning; keep frequent rule editing in frontend | governance/backend + frontend | 2026-05-19 | WinUI quality rule set editor with list/save/delete plus save-from-canvas graph rule extraction parity landed on 2026-05-29 |
...
| Baseline compare / lineage helper surfaces | covered (`compat-hidden`) | `renderer/workflow/support-ui-*.js` | covered | backendize lineage/compare computation; frontend remains a viewer/filter surface | workflow/governance backend + frontend | 2026-05-19 | WinUI baseline list/save/compare viewer plus compare report export, lineage export, and local run-audit backed historical lineage/query parity landed on 2026-06-02 |
...
| Electron offline home shell | still useful for offline queue, precheck, and local release-helper workflows during transition | explicit offline-home compatibility helper shell; keep off primary onboarding and release path |
